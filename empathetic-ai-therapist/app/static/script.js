// State variables
let currentUserId = null, mediaRecorder = null, wellnessData = null, activeTask = null, timerInterval = null, timerRemaining = 0;
...
function updateSessionUI() {
    const sessionId = sessionStorage.getItem('healaura_session_id');
    if (sessionId) {
        dom.sessionSetup.classList.add('hidden');
        dom.recordingControls.classList.remove('hidden');
        dom.sessionIdEl.textContent = `Session: ...${sessionId.slice(-6)}`;
        dom.endSessionBtn.disabled = false;
    } else {
        dom.sessionSetup.classList.remove('hidden');
        dom.recordingControls.classList.add('hidden');
        dom.sessionIdEl.textContent = `(no active session)`;
        ...
        dom.conversation.innerHTML = `
            <div class="turn ai">
                ...
                <div class="msg-content">
                    <div class="speaker">AI Therapist</div>
                    <div class="msg-bubble">Your session has been restored. Please continue where you left off.</div>
                </div>
            </div>`;
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    // --- Populate HTML and assign DOM elements ---
    Object.assign(dom, {
        appContainer: document.getElementById('appContainer'), authContainer: document.getElementById('authContainer'),
    ...
    // Inject and assign event listeners for dynamically added content
    setupAuthUI();
    setupTherapyUI();
    setupWellnessUI();
});
...
function setupTherapyUI() {
    ...
    Object.assign(dom, {
        sessionSetup: document.getElementById('sessionSetup'), languageSelect: document.getElementById('languageSelect'),
        startSessionBtn: document.getElementById('startSession'), endSessionBtn: document.getElementById('endSession'),
        recordingControls: document.getElementById('recordingControls'),
        startRecBtn: document.getElementById('startRec'), stopRecBtn: document.getElementById('stopRec'),
        submitRecBtn: document.getElementById('submitRec'), conversation: document.getElementById('conversation'),
        statusIndicator: document.getElementById('statusIndicator'), recStatus: document.getElementById('recStatus'),
        taskControls: document.getElementById('taskControls'), markDoneBtn: document.getElementById('markDone'),
        timerBox: document.getElementById('timerBox'), timerValue: document.getElementById('timerValue'),
    });
...
function setupWellnessUI() {
    dom.wellnessContainer.innerHTML = `<h2 style="font-size:26px; margin-bottom:24px; color:var(--text-primary);">Your Wellness Dashboard</h2><div class="card"><div class="card-header"><h3>Daily Logging</h3></div><div class="card-content"><h4>Part 1: Quick Thoughts</h4><p class="small" style="margin-bottom:16px;">How are you feeling? Click an hour to log your thoughts and emotions. Logged hours are disabled.</p><div class="hourly-logger" id="hourlyLogger"></div><hr style="margin: 28px 0; border: none; border-top: 1px solid var(--border);"><h4>Part 2: Daily Reflection</h4><p class="small" style="margin-bottom:16px;">Fill this out once per day to find deeper patterns.</p><form id="dailyReflectionForm" class="daily-reflection-form"></form></div></div><div class="card"><div class="card-header"><h3>This Week's Goals</h3></div><div class="card-content goals-grid" id="goalsContainer"></div></div><div class="card"><div class="card-header"><h3>AI-Powered Insights</h3></div><div class="card-content"><div class="insights-grid" id="insightsContainer"></div></div></div><div class="charts-grid"><div class="card"><div class="card-header"><h3>Sleep Quality vs. Sentiment</h3></div><div class="card-content"><canvas id="sleepChart"></canvas><a class="interpret-btn" onclick="toggleInterpret(this)">How to Interpret ▼</a><div class="interpret-text hidden">This chart shows your average mood based on sleep quality. A green bar means the activity is linked to a positive mood on average, while a red bar suggests a link to a negative mood. The height of the bar shows the strength of this connection.</div></div></div><div class="card"><div class="card-header"><h3>Activity vs. Sentiment</h3></div><div class="card-content"><canvas id="activityChart"></canvas><a class="interpret-btn" onclick="toggleInterpret(this)">How to Interpret ▼</a><div class="interpret-text hidden">This compares how different activities correlate with your mood. A green bar means the activity is linked to a positive mood on average, while a red bar suggests a link to a negative mood. The height of the bar shows the strength of this connection.</div></div></div></div><div class="charts-grid"><div class="card"><div class="card-header"><h3>Social vs. Sentiment</h3></div><div class="card-content"><canvas id="socialChart"></canvas><a class="interpret-btn" onclick="toggleInterpret(this)">How to Interpret ▼</a><div class="interpret-text hidden">This chart shows the connection between who you spend time with and your overall mood. A green bar means the interaction is linked to a positive mood on average, while a red bar suggests a link to a negative mood. The height of the bar shows the strength of this connection.</div></div></div><div class="card"><div class="card-header"><h3>Sentiment Over Time</h3></div><div class="card-content"><canvas id="sentimentOverTimeChart"></canvas><a class="interpret-btn" onclick="toggleInterpret(this)">How to Interpret ▼</a><div class="interpret-text hidden">This line chart tracks your average daily mood. Upward trends indicate periods of positivity, while downward trends might highlight challenging times. It helps you see your emotional journey at a glance.</div></div></div></div><div class="charts-grid"><div class="card"><div class="card-header"><h3>Emotion Distribution</h3></div><div class="card-content"><canvas id="emotionDistributionChart"></canvas><a class="interpret-btn" onclick="toggleInterpret(this)">How to Interpret ▼</a><div class="interpret-text hidden">This chart shows the percentage of each emotion you've logged. It gives a high-level overview of your most frequent feelings. Are there any surprises here?</div></div></div><div class="card"><div class="card-header"><h3>Topic Word Cloud</h3></div><div class="card-content"><div id="wordCloudContainer"></div><a class="interpret-btn" onclick="toggleInterpret(this)">How to Interpret ▼</a><div class="interpret-text hidden">The bigger the word, the more often it appears in your logs. This helps you quickly see the topics that are on your mind the most, offering clues about what drives your emotions.</div></div></div></div><div class="charts-grid"><div class="card"><div class="card-header"><h3>Daily Sentiment (Last 30 Days)</h3></div><div class="card-content"><div id="calendarHeatmap"></div><a class="interpret-btn" onclick="toggleInterpret(this)">How to Interpret ▼</a><div class="interpret-text hidden">Greener days are more positive, redder days are more negative. Click any day to get an AI-generated summary of your logs for that day.</div></div></div><div class="card"><div class="card-header"><h3>Recent Session Summaries</h3></div><div class="card-content"><ul class="session-summary-list" id="sessionSummaryList">(No session summaries found)</ul></div></div></div>`;
    Object.assign(dom, {
        hourlyLogger: document.getElementById('hourlyLogger'), insightsContainer: document.getElementById('insightsContainer'),
        goalsContainer: document.getElementById('goalsContainer'), dailyReflectionForm: document.getElementById('dailyReflectionForm'),
        sessionSummaryList: document.getElementById('sessionSummaryList'), calendarHeatmap: document.getElementById('calendarHeatmap'),
...
// --- AUTH & SESSION EVENT HANDLERS (ROBUST VERSION) ---
...
function onLogout() {
    ...
    currentUserId = null;
    wellnessData = null;
    sessionStorage.removeItem('healaura_session_id');
...
async function onStartSession() {
    showLoader(dom.startSessionBtn, true);
    updateStatus("Starting...", true);
    try {
        const form = new FormData();
        form.append('user_id', currentUserId);
        form.append('language', dom.languageSelect.value);
        ...
        sessionStorage.setItem('healaura_session_id', j.session_id);
        updateSessionUI();
        dom.conversation.innerHTML = "";
        appendTurn('ai', { reply_structured: j.initial_reply, tts_b64: j.tts_b64 }, null, "start");
        wellnessData = null;
        clearActiveTask();
        ...
        showLoader(dom.startSessionBtn, false);
    }
}
...
async function onEndSession() {
    const sessionId = sessionStorage.getItem('healaura_session_id');
    if (!sessionId) return;
    showLoader(dom.endSessionBtn, true);
    updateStatus("Ending...", true);
    try {
        const form = new FormData();
        form.append('session_id', sessionId);
        const res = await fetch('/end_session', { method: 'POST', body: form });
        const j = await res.json();
        appendTurn('ai', { reply_structured: j.closing_reply, tts_b64: j.tts_b64 }, null, "end");
    } catch (e) {
        ...
        updateSessionUI();
        clearActiveTask();
        showLoader(dom.endSessionBtn, false);
...
async function onSubmitRecording() {
    const sessionId = sessionStorage.getItem('healaura_session_id');
    if (!audioChunks.length || !sessionId) {
        showAlert('Session Error', 'No active session ID found or no audio recorded. Please start a new session.', 'error');
        return;
    }
    ...
    try {
        ...
        if (!res.ok) {
            let errorDetail = 'Failed to process audio. The server responded with an error.';
            try { errorDetail = (await res.json()).detail; } catch (e) {}
        ...
        showAlert('Processing Error', e.message, 'error');
    } finally {
...
async function onSignup(e) { e.preventDefault(); const btn = dom.signupForm.querySelector('button'); showLoader(btn, true); try { const form = new FormData(dom.signupForm); const res = await fetch('/signup', { method: 'POST', body: form }); const data = await res.json(); if (!res.ok) throw new Error(data.detail); showAlert('Success', 'Account created. Please log in.', 'success'); dom.signupForm.reset(); dom.showLoginLink.click(); } catch (err) { showAlert('Signup Error', err.message, 'error'); } finally { showLoader(btn, false); } }
...
async function onTaskDone() { const sessionId = sessionStorage.getItem('healaura_session_id'); if (!sessionId || !activeTask) return; showLoader(dom.markDoneBtn, true); try { const form = new FormData(); form.append('session_id', sessionId); form.append('task_id', activeTask.task_id); const res = await fetch('/task_done', { method: 'POST', body: form }); if (!res.ok) throw new Error(await res.text()); const j = await res.json(); appendTurn('ai', { reply_structured: j.reply, tts_b64: j.tts_b64 }, null, j.decision.decision); } catch (e) { showAlert('Task Error', e.message, 'error'); } finally { showLoader(dom.markDoneBtn, false); clearActiveTask(); } }
...
function renderDailyLogging(logs) {
    ...
    for (let i = 0; i < 24; i++) {
        ...
        btn.onclick = (e) => showHourlyLogModal(i, e.currentTarget);
...
function showHourlyLogModal(hour, buttonEl) {
    ...
    form.onsubmit = async (e) => {
        ...
        if (newLog) {
            ...
            if (wellnessData && wellnessData.raw_logs) wellnessData.raw_logs.push(newLog);
        }
        modal.remove();
    };
}
...
function renderInsights(insights) { dom.insightsContainer.innerHTML = `<div class="insight-card insight-notice"><h4>✨ We've Noticed...</h4><p>${insights.causal_inference || 'N/A'}</p></div><div class="insight-card insight-positive"><h4>✅ Positive Pattern</h4><p>${insights.positive_pattern || 'N/A'}</p></div><div class="insight-card insight-negative"><h4>⚠️ Challenge Pattern</h4><p>${insights.challenge_pattern || 'N/A'}</p></div><div class="insight-card insight-growth"><h4>🌱 Area for Growth</h4><p>${insights.area_for_growth || 'N/A'}</p></div><div class="insight-card insight-action" style="grid-column: 1 / -1;"><h4>💡 Actionable Suggestion</h4><p>${insights.actionable_suggestion || 'N/A'}</p></div>`; }
function renderGoals(goals) { dom.goalsContainer.innerHTML = (goals && goals.length > 0) ? goals.map(g => `<div class="goal-card"><div class="goal-card-content"><span class="goal-card-icon">🎯</span><p>${g.goal}</p></div>${g.rationale ? `<div class="rationale">${g.rationale}</div>` : ''}</div>`).join('') : '<p>Your personalized goals will appear here soon.</p>'; }
...
function renderCorrelationChart(canvasId, data, label) {
    ...
    if (!data || Object.keys(data).every(k => data[k] === null)) {
        ctx.font = "14px Inter";
    ...
    const sortedLabels = desiredOrder.filter(label => data[label] !== null);
    const sortedValues = sortedLabels.map(label => data[label]);
    const maxAbsValue = Math.max(0.2, ...sortedValues.map(v => Math.abs(v)));
    const scaleMax = maxAbsValue * 1.2;
    ...
    charts[canvasId] = new Chart(ctx, {
        ...
        data: {
            labels: sortedLabels,
            datasets: [{ label: label, data: sortedValues, backgroundColor: backgroundColors, borderColor: borderColors, borderWidth: 1, maxBarThickness: 60 }]
        },
        options: {
            indexAxis: 'y',
            scales: { x: { min: -scaleMax, max: scaleMax } },
